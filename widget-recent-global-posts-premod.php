...
if ( !function_exists( 'rgpwidget_exclude_blogs' ) ) :
	...
	function rgpwidget_exclude_blogs( $where, Network_Query $query ) {
		...
		if( !empty( $query->query_vars['blogs_in'] ) ){
			return $where . sprintf( ' AND %s.BLOG_ID IN (%s) ', $query->network_posts, implode( ', ', (array)$query->query_vars['blogs_in'] ) );
		}elseif( !empty( $query->query_vars['blogs_not_in'] ) ){
			return $where . sprintf( ' AND %s.BLOG_ID NOT IN (%s) ', $query->network_posts, implode( ', ', (array)$query->query_vars['blogs_not_in'] ) );
		}else{
			return $where;
		}
...
class Recent_Global_Posts_Widget extends WP_Widget {
			...
			if ( network_have_posts() ) :
				...
				while ( network_have_posts() ) :
					...
					echo '<li>';
						...
						if ( $recentglobalpostsimage == 'show' ) :
							echo '<div class="fgthumb">';
						...
						switch ( $recentglobalpostsdisplay ) {
							...
							case self::DISPLAY_TITLE_BLOG_CONTENT:
								echo '<h4><a href="', $the_permalink, '">', $title, '[', $blog_title, ']</a></h4>';
								echo $content;
								echo '<a href="', $the_permalink, '" class="rmlink">', __( 'Read More', 'rgpwidget' ), '</a>';
								break;
							case self::DISPLAY_TITLE_CONTENT:
							default:
								echo '<h4><a href="', $the_permalink, '">', $title, '</a></h4>';
								echo $content;
								echo '<a href="', $the_permalink, '" class="rmlink">', __( 'Read More', 'rgpwidget' ), '</a>';
								break;
								echo '</div>';
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'recentglobalpostsimage' ) ?>"><?php _e( 'Featured Image?', 'rgpwidget' ) ?>:</label>
			<select id="<?php echo $this->get_field_id( 'recentglobalpostsimage' ) ?>" class="widefat" name="<?php echo $this->get_field_name( 'recentglobalpostsimage' ) ?>">
				<option value="show"<?php selected( $instance['recentglobalpostsimage'], 'show' ) ?> ><?php _e( 'Show', 'rgpwidget' ) ?></option>
				<option value="hide"<?php selected( $instance['recentglobalpostsimage'], 'hide' ) ?> ><?php _e( 'Hide', 'rgpwidget' ) ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'include_blogs' ) ?>"><?php _e( 'Include Blogs', 'rgpwidget' ) ?>:</label>
			<input type="text" id="<?php echo $this->get_field_id( 'include_blogs' ) ?>" class="widefat" name="<?php echo $this->get_field_name( 'include_blogs' ); ?>" value="<?php echo esc_attr( $instance['include_blogs'] ) ?>"><br>
			<small><?php esc_html_e( 'Blog IDs, separated by commas.', 'rgpwidget' ) ?></small>
		</p>
